...
export function LinkPage({ pathname, pageLink }: { pathname: string; pageLink: PageLink }) {
  ...
  const isCurrentPage = pageLink.startWith ? pathname.startsWith(href) : href === pathname;
...
export function LinkSheet({ pathname, label, href }: { pathname: string; label: string | JSX.Element; href: string }) {
