...
import { UsePublicationQuery, UsePublicationsQueryByAuthor } from '@/hooks/use-publication';
...
export default function PublicationsList({ initialData }: PublicationsListProps) {
  const { data: publications } = UsePublicationsQueryByAuthor({ initialData, authorId: initialData[0].authorId });

  return (
    <article className='space-y-4'>
      {publications.map((pub) => (
        <CardPublication key={pub.id} initialData={pub} />
      ))}
    </article>
  );
}
...
function CardPublication({ initialData }: CardPublicationProps) {
